...
def products():
    ...
    while has_more_prices:
        price_list = stripe.Price.list(
            limit=100, active=True, starting_after=starting_after)
    ...
    for product in products:
        skus[product.id] = [
            price for price in prices if price.product == product.id]
...
def webhook_received():
    ...
    cursor = db.cursor()
    cursor.execute(
        'SELECT event_id FROM webhooks WHERE event_id = %s', (event_id,))
    stored_event = cursor.fetchone()

    if stored_event:
        current_app.logger.info(
            f" Event has already been processed.\nEvent: {event.type}")
        return jsonify(success=True)
    else:
        cursor.execute(
            'INSERT INTO webhooks(event_id) VALUES (%s)', (event_id,))
        ...
        if event.type == 'checkout.session.completed':
            ...
            for item in line_items:
                ...
                item = {"Id": item.price.id, "price": item.price.unit_amount /
                        100, "quantity": item.quantity, "metadata": metadata}
            ...
            try:
                ...
                formatted_order_string = '\n'.join(
                    str(item) for item in formatted_order)
                formatted_order_string = formatted_order_string.replace(
                    "'", "").replace("{", "").replace("}", "")
            ...
            if user is not None:
                ...
                cursor.execute("""UPDATE users SET subscription_status = %s,subscription_plan=%s WHERE customer_id=%s  """, (
                    subscription_status, price_id, customer_id))

                current_app.logger.info(
                    f" Successfully saved {user[1]}'s details.\nsubscription_status: {subscription_status}\nprice_id: {price_id}")

            else:

                current_app.logger.warning(
                    f" Subcription details could not be saved")
            ...
            if user and user[5] is not None:
                if user[5] != customer_id:
                    current_app.logger.info(
                        f" User with Customer ID: {user[5]} already in the system, has been assigned new Stripe customer with ID: {customer_id}")
                else:
                    current_app.logger.info(
                        f" User with Customer ID: {customer_id} already in the system")
            ...
            current_app.logger.info(
                f' Customer with id: {customer_id} succesfully deleted from STRIPE')
            ...
            cursor = db.cursor()
            cursor.execute(
                'SELECT * FROM users WHERE customer_id = %s ', (customer_id,))
            user = cursor.fetchone()
            ...
            if user is not None:
                ...
                current_app.logger.info(
                    f" Customer subscription Successfully Updated\n User: {user[1]}'s details.\nSubscription Status: {subscription_status}\nPrice_id: {price_id}")
            ...
            if user is not None:
                cursor.execute(
                    'UPDATE users SET subscription_plan=%s, customer_id=%s, subscription_status=%s WHERE id=%s ', (None, None, None, user_id))
                db.commit()
                current_app.logger.info(
                    f' Customer subscription {status}, plan succesfully deleted from Database')
                ...
                try:
                    ...
                    error_msg = str(e)
                    current_app.logger.warning(
                        f'The following error has ocurred: \n{error_msg}')
            ...
            current_app.logger.info(
                f' Customer subscription schedule canceled, customer id : {customer_id}')
            ...
            if customer_id is not None:
                ...
                try:
                    ...
                    billing_session = stripe.billing_portal.Session.create(
                        ...
                        locale='auto',
                        return_url=url_for(
                            'stripe.customer_portal_redirect', _external=True)
                    )
