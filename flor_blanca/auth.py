...
from flor_blanca.postDb import get_user_by_email
    ...
    if request.method == 'POST':
        ...
        if error is None:
            ...
            if session['email'] in ADMIN_LIST:
                ...
                session['role'] = 'user'
            flash(('Sesión iniciada con éxito.', 'primary'))
...
def logout():
    ...
    session.clear()
    flash(('Sesión cerrada con éxito.', 'primary'))
...
def save_question_count(count):
    ...
    cursor.execute(
        'UPDATE users SET used_questions=%s WHERE email = %s', (count, email))
    current_app.logger.info(
        f" Question count saved correctly\nValues\nUsed Questions: {count}")
...
def login_required(view):
    ...
    def wrapped_view(**kwargs):
        if g.user is None:
            flash(('Inicia una sesión.', 'primary'))
            return redirect(url_for('auth.login'))
...
def required_basic(view):
    ...
    def wrapped_view(**kwargs):
        ...
        cursor = db.cursor()
        cursor.execute(
            'SELECT customer_id FROM users WHERE email = %s', (email,))
        user = cursor.fetchone()
        customer_id = user[0]

        if customer_id is None:
            message = 'No puedes acceder a los servicios de Tienda,  Preguntas, planes de Personalidad,  Alma o Espíritu, Tarot y Sesión en Directo.\nPara tener acceso a estos servicios debes comprar un plan de suscripción. Si tienes más dudas consulta nuestra sección de Ayuda.'
...
def required_spirit_plan(view):
    ...
    def wrapped_view(**kwargs):
        ...
        if role == 'admin':
            ...
            if subscription_plan is None or subscription_plan[0] != "price_1Nk5YdAEZk4zaxmwEqqHWSS2":
                message = 'No puedes acceder a los servicios de Tienda,  Preguntas, planes de Personalidad,  Alma o Espíritu, Tarot y Sesión en Directo.\nPara tener acceso a estos servicios debes comprar un plan de suscripción. Si tienes más dudas consulta nuestra sección de Ayuda.'
                flash((message, 'warning'))
                return redirect(url_for('index'))
...
def required_soul_plan(view):
    ...
    def wrapped_view(**kwargs):
        ...
        if subscription_plan is None or subscription_plan[0] != 'price_1Nk5YdAEZk4zaxmwEqqHWSS2' and subscription_plan[0] != 'price_1Nk5YMAEZk4zaxmws0AhQfIs':
            message = 'No puedes acceder a los servicios de Tienda,  Preguntas, planes de Personalidad,  Alma o Espíritu, Tarot y Sesión en Directo.\nPara tener acceso a estos servicios debes comprar un plan de suscripción. Si tienes más dudas consulta nuestra sección de Ayuda.'
            flash((message, 'warning'))
            return redirect(url_for('index'))
...
def reset_password():
    if request.method == 'POST':
        ...
        if user is not None:
            ...
            cursor = db.cursor()
            cursor.execute(
                'UPDATE users set password_recovery=%s WHERE email=%s', (secret_token, email))
            ...
            try:
                ...
                flash((f'El código ha sido enviado a {email}', 'success'))
            ...
            flash(
                (f'La dirección de correo {email} no existe en nuestra base de datos', 'danger'))
...
def newPassword():
    ...
    return render_template('auth/newPassword.html', email=email)
...
def validate_cookie(cookie, email):
    ...
    cursor = db.cursor()
    cursor.execute(
        'SELECT password_recovery FROM users WHERE email=%s', (email,))
...
def delete_account():
    ...
    if request.method == 'POST':
        ...
        if user[0] is None and user[1] is None and user[2] is None:
            ...
            current_app.logger.info(
                f'User with email: {email} succesfully deleted from Database')
            session.clear()
